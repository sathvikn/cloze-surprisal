...
add_k = float(sys.argv[2])
possible_V = int(sys.argv[3])
...
word_responses = (out.clozeprob * out.Total_Response_Count).apply(np.ceil)
numerator = word_responses + add_k
denominator = out.Total_Response_Count + (add_k * possible_V)
out.clozeprob = numerator / denominator
